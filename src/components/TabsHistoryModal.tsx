...
export function TabsHistoryModal() {
    const { isModalOpen, closeModal } = useTabsHistoryModal();
    const { data: userCodes = [] } = useUserCodesQuery();

    const tabsHistory = userCodes.map((code: any) => ({
        id: code.id,
        code: code.code,
        value: code.value,
        date: code.createdAt,
    ...
    return (
        <Dialog open={isModalOpen} onClose={closeModal} maxWidth="md" fullWidth sx={{ zIndex: 1500 }}>
            <DialogTitle
                sx={{
                    display: 'flex',
                    alignItems: 'center',
            ...
            <DialogContent sx={{ py: 3 }}>
                            ...
                            <TableBody>
                                {tabsHistory.map((item: any) => (
                                    <TableRow key={item.id} hover>
                                        ...
                                        <TableCell align="center">
                                            <Typography variant="body2">{new Date(item.date).toLocaleDateString('pt-BR')}</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                ...
                {tabsHistory.length > 0 && (
                    <Box sx={{ mt: 3, p: 2, bgcolor: 'background.default', borderRadius: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            <strong>Valor total acumulado:</strong> R$ {tabsHistory.reduce((sum: any, item: any) => sum + item.value, 0).toFixed(2).replace('.', ',')}
                        </Typography>
                    </Box>
                )}
            </DialogContent>
